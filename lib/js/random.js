...
/**
 * Generate pseudo-random bytes from a reliable source
 * @param {Number} size
 * @returns {Uint8Array}
 */

exports.getSubtleRandomValues = async function getSubtleRandomValues(size) {
  const crypto = global.crypto || global.msCrypto;

  // Immutable read-only crypto interface, can not be polyfilled by malicious JS
  // https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto
  if (!crypto.subtle || typeof crypto.subtle !== 'object')
    throw new Error('Not supported.');

  if (size <= 32) {
    // subtle only generates random keys, not random bytes
    ...
    let fullArray = new Uint8Array(size);
    const array = await getSubtleRandomValues(32);
    const subarray = await getSubtleRandomValues(size - 32);
    fullArray.set(array, 0);
    fullArray.set(subarray, 32);
    return fullArray;
  }
}

/**
 * Generate pseudo-random buffer from a reliable source
 * @param {Number} size
 * @returns {Buffer}
 */

exports.getSubtleRandomBuffer = async function getSubtleRandomBuffer(size) {
  const array = await exports.getSubtleRandomValues(size);
